...
def Visfeature(args, model, captuer, inputs, v_path=None, weight_softmax=None, FusionNet=False):
    # TSNE cluster
    if FusionNet:
        ...
        tsne = manifold.TSNE(perplexity=10, n_components=2, init='pca', n_iter=5000)
    ...
    imgs = []
    for img in inputs:
        img = make_grid(img[:args.batch_size], nrow=int(np.sqrt(args.batch_size)), padding=2).unsqueeze(0)
        imgs.append(img)
    imgs = torch.cat(imgs)

    fig = plt.figure()
    ax = fig.add_subplot()
    sns.heatmap(torch.sum(make_grid(imgs.detach(), nrow=int(imgs.size(0) ** 0.5), padding=2), dim=0).cpu().numpy(),
        annot=False, fmt='g', ax=ax)
    ax.set_title('input', fontsize=10)
    fig.savefig(os.path.join(args.save, 'input.jpg'), dpi=fig.dpi)
    plt.close()
    ...
    heatmaps = []
    for heat in headmap:
        heat = make_grid(heat[:args.batch_size], nrow=int(np.sqrt(args.batch_size)), padding=2)[0].unsqueeze(0)
        heatmaps.append(heat)
    heatmaps = torch.cat(heatmaps)

    fig = plt.figure()
    ax = fig.add_subplot()
    sns.heatmap(torch.sum(make_grid(heatmaps.detach(), nrow=int(heatmaps.size(0) ** 0.5), padding=2), dim=0).cpu().numpy(),
        annot=False, fmt='g', ax=ax)
    ax.set_title('heatmaps', fontsize=10)
    fig.savefig(os.path.join(args.save, 'heatmaps.jpg'), dpi=fig.dpi)
    plt.close()

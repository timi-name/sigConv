...
def build_scheduler(args, optimizer):
    if args.scheduler['name'] == 'cosin':
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1,
                                                               ...
                                                               eps=1e-08)
    elif args.scheduler['name'] == 'StepLR':
...
class SoftTargetCrossEntropy(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss = torch.sum(-target_one_hot * F.log_softmax(x, dim=-1), dim=-1)
        return loss.mean()
...
#
#     def forward(self, x):
#         Spatial_out, Temporal_out = x
#
#         T = self.args.temper
#         outputs_S = F.softmax(Temporal_out[0][0] / T, dim=-1)
#         outputs_T = F.log_softmax(Spatial_out[0] / T, dim=-1)
#         distill_loss = self.criterion(outputs_S, outputs_T)*T*T
#         return distill_loss
#
# class RCM_loss(torch.nn.Module):
#     def __init__(self, args, model: torch.nn.Module, alpha=0.5):
#         super(RCM_loss, self).__init__()
#         self.args = args
#         self.alpha = alpha
#
...
#             F.log_softmax(temp_w / T, dim=1),
#             F.softmax(temp_w / T, dim=1),
#             reduction='batchmean'
#         ) * (T ** 2)
#
#         # 时间蒸馏损失
#         temporal_loss = F.kl_div(
#             F.log_softmax(target_weight / T, dim=1),
#             F.softmax(target_weight / T, dim=1),
#             reduction='batchmean'
#         ) * (T** 2)
#
#         # 硬标签损失
#         hard_loss = F.cross_entropy(torch.cat([spatial_student, temporal_student], dim=1), labels)
#
#         # 结合所有损失
#         total_loss = self.alpha * (spatial_loss + temporal_loss) + (1 - self.alpha) * hard_loss
#         return total_loss
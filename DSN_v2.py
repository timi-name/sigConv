...
class SMSBlock(nn.Module):
    def __init__(self, channel_list, kernel_size=None, stride=None, padding=0, name='i3d'):
        ...
        self.end_points[end_point] = SpatialInceptionModule(sum([hidden_channels[0], hidden_channels[2],
                                                                 hidden_channels[4], hidden_channels[5]]), out_channels,
                                                            name + end_point)
        if kernel_size is not None:
            end_point = 'MaxPool3d_3x3'
            self.end_points[end_point] = MaxPool3dSamePadding(kernel_size=kernel_size, stride=stride, padding=padding)
        self.build()
...
class DSNNetV2(nn.Module):
    def __init__(self,
                 # DSN
                 args, num_classes=400, spatial_squeeze=True, name='inception_i3d', in_channels=3,
        ...
        self.args = args
        self.sms_depth = sms_depth
        # self.cbam = CBAM(64)
        self.stem = nn.Sequential(
            Unit3D(in_channels=in_channels, out_channels=64, kernel_size=[1, 7, 7],
                   stride=(1, 2, 2), padding=(0, 3, 3), name=name + 'Conv3d_1a_7x7'),
            MaxPool3dSamePadding(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=0),
            # self.cbam,
            Unit3D(in_channels=64, out_channels=64, kernel_size=[1, 1, 1], padding=0,
                   name=name + 'Conv3d_2b_1x1'),
            Unit3D(in_channels=64, out_channels=192, kernel_size=[1, 3, 3],
        ...
        # assert len(sms_block) == self.sms_depth
        self.SMS_layers = nn.ModuleList([])
        self.SMS_layers.append(SMSBlock(sms_block[0], kernel_size=[1, 3, 3], stride=(1, 3, 3), padding=0))
        self.SMS_layers.append(nn.Dropout(dropout_spatial_prob)),
    ...
    def forward(self, x, garr=None):
        ...
        self.feat = x.data
        x = self.avg_pool3d(x).view(x.size(0), x.size(1), -1).permute(0, 2, 1)
        x = self.LinearMap(x)
